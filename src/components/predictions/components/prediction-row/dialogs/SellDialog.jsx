  ...
  return (
    <Dialog open={sellPrompt} onOpenChange={setSellPrompt}>
      <DialogTrigger asChild>
        <Button onClick={() => setSellPrompt(true)} className="bg-rose-600 hover:bg-rose-700 text-foreground border-0 shadow-md shadow-rose-500/20 w-full">{t("Predictions:sell")}</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] bg-card ring-1 dark:ring-white/[0.08] ring-border border-border/60 text-foreground shadow-2xl dark:shadow-black/60 shadow-black/25">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-semibold text-foreground">{t("Predictions:sellDialog.title")}</DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground">{t("Predictions:sellDialog.description")}</DialogDescription>
            </div>
          </div>
        </DialogHeader>
            ...
            <CollapsibleContent>
              <div className="rounded-xl border border-border bg-accent/30 dark:bg-white/[0.05] px-4 py-3">
                <div className="grid grid-cols-4 gap-3 text-center">
                  <div>
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">{t("Predictions:sellDialog.oddsFractional", { defaultValue: "Fraction" })}</div>
                    <div className="text-sm font-semibold text-foreground">{fracNum}:{fracDen}</div>
                  </div>
                  <div>
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">{t("Predictions:sellDialog.oddsDecimal", { defaultValue: "Decimal" })}</div>
                    <div className="text-sm font-semibold text-foreground">{decimalOddsNo.toFixed(2)}</div>
                  </div>
                  <div>
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">{t("Predictions:sellDialog.oddsAmerican", { defaultValue: "American" })}</div>
                    <div className="text-sm font-semibold text-foreground">{americanOddsNo > 0 ? "+" : ""}{americanOddsNo}</div>
                  </div>
                  <div>
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">{t("Predictions:sellDialog.oddsProbability", { defaultValue: "Implied prob." })}</div>
                    <div className="text-sm font-semibold text-foreground">{impliedProbabilityNo.toFixed(1)}%</div>
                  </div>
                </div>
              </div>
            </CollapsibleContent>
                ...
                <div className="rounded-xl border border-border bg-accent/30 dark:bg-white/[0.05] px-4 py-3 space-y-2">
                  {marketStats.bestBid != null && marketStats.buyOrderCount > 0 ? (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">{t("Predictions:sellDialog.marketBestBid", { defaultValue: "Best bid" })} <span className="text-muted-foreground/60">({t("Predictions:sellDialog.trueBet", { defaultValue: "true bet" })})</span></span>
                  ...
                  {marketStats.bestAsk != null && marketStats.sellOrderCount > 0 ? (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">{t("Predictions:sellDialog.marketBestAsk", { defaultValue: "Best ask" })} <span className="text-muted-foreground/60">({t("Predictions:sellDialog.falseBet", { defaultValue: "false bet" })})</span></span>
                  ...
                  {marketStats.latestTrade != null && marketStats.latestTrade > 0 ? (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">{t("Predictions:sellDialog.marketLastTrade", { defaultValue: "Last trade" })}</span>
          ...
          <Collapsible defaultOpen>
            ...
            <CollapsibleContent>
              <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 px-4 py-3">
                <div className="text-xs dark:text-rose-200/60 text-rose-700/60 mb-0.5">{t("Predictions:sellDialog.outcomeIfWin", { defaultValue: "If the prediction resolves in your favour (False)" })}</div>
                <div className="text-sm font-semibold dark:text-rose-100 text-rose-700">{formatAmount(totalBackingIfNo, _backingPrecision)} {res.backingAsset.symbol}</div>
                <div className="mt-1 text-[11px] dark:text-rose-100/60 text-rose-700/60">
