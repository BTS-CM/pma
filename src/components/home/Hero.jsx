...
import { ArrowRight, PlusCircle, ChevronDown } from "lucide-react";

import FeaturedStrip from "./FeaturedStrip";

export default function Hero(properties) {
  const { t } = useTranslation(locale.get(), { i18n: i18nInstance });
  const { liveStats } = properties;

  return (
    <section
      className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-indigo-900/40 via-slate-900/40 to-fuchsia-900/30 p-6 sm:p-10"
    ...
    >
      ...
      <div className="relative max-w-3xl mx-auto text-center">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-white leading-[1.1] [text-shadow:_0_1px_2px_rgba(0,0,0,0.6)]">
          {t("Home:hero.title")}
        </h1>
        ...
        <FeaturedStrip liveStats={liveStats} />
        <a
          href="#jump-in"
